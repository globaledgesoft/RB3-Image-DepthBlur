    ...
    for (uint32_t i = 0;i < result->num_output_buffers ;i++) {
        ...
        if (stream->streamType == CAMERA3_TEMPLATE_STILL_CAPTURE) {
            ...
            if (!(!mConfig->mZslEnabled && result->frame_number < NONZSL_STABLE_FRAME_COUNT)
				...
				//background_blur
				sprintf(main_cam_img, "%s/camera_capture_%d_%4d_%02d_%02d_%02d_%02d_%02d.%s",
                    CAMERA_STORAGE_DIR, result->frame_number,t->tm_year+1900,t->tm_mon+1,
                    t->tm_mday,t->tm_hour,t->tm_min,t->tm_sec,"jpg");
                ...
                if (jpegBlob.JPEGBlobId == JPEG_BLOB_ID) {
                    ...
                    TEST_ERR("Failed to Get Picture size:%d\n",size);
                    FILE* fd = fopen(fname, "w");
                    fwrite(data, size, 1, fd);
                    fclose(fd);
                }
                TEST_INFO("Get One Picture %d Last\n",mSnapshotNum);
                if (!mConfig->mZslEnabled && mSnapshotNum == 0) {
                    pthread_mutex_lock(&mCaptureSyncMutex);
                    pthread_cond_signal(&mCaptureSyncCond);
                    pthread_mutex_unlock(&mCaptureSyncMutex);
...
int QCameraHAL3TestSnapshot::initSnapshotStreams()
{
    ...
    if (mConfig->mZslEnabled){
        previewStream.usage = GRALLOC1_CONSUMER_USAGE_HWCOMPOSER | GRALLOC1_CONSUMER_USAGE_GPU_TEXTURE;
    } else {
...
#if 0
    if (0 == mConfig->mCameraId) {
	previewStream.usage |= GRALLOC_USAGE_PRIVATE_3;
	printf("%s:%d set preview usage %x for camera id %d\n",
		__FUNCTION__, __LINE__, previewStream.usage, mConfig->mCameraId);
    }
#endif
    ALOGI("===[%s][%d]===[%s]=\n",__func__,__LINE__,mConfig->mAltekSensor?("altek sensor"):("not altek"));

    if (mConfig->mAltekSensor) {
	previewStream.usage |= GRALLOC1_CONSUMER_USAGE_FOREIGN_BUFFERS;
	printf("%s:%d set preview usage %x for camera id %d\n",
		__FUNCTION__, __LINE__, previewStream.usage, mConfig->mCameraId);
    }
    ...
    if (res < 0 || outputSnapshotStreams.size() == 0) {
        TEST_ERR("Failed to find output stream for snapshot: w: %d, h: %d, fmt: %d",
    ...
    snapshotStream.usage = GRALLOC1_CONSUMER_USAGE_CPU_READ_OFTEN;
...
{
    int res = 0;
    TEST_INFO("preinit snapshot streams start\n");
    camera3_stream_t previewStream;
    camera3_stream_t snapshotStream;
    std::vector<camera3_stream_t*> streams;
    int stream_num = 2;
    ...
    if (mConfig->mZslEnabled) {
        previewStream.usage =
            GRALLOC1_CONSUMER_USAGE_HWCOMPOSER | GRALLOC1_CONSUMER_USAGE_GPU_TEXTURE;
    } else {
        previewStream.usage = GRALLOC1_CONSUMER_USAGE_NONE;
    }
    previewStream.rotation = 0;
    ...
    TEST_INFO("preinit snapshot streams end\n");
    return res;
}
...
int QCameraHAL3TestSnapshot::RequestCaptures(StreamCapture request)
{
    ...
    if (!mConfig->mZslEnabled) {
       ...
       if (pthread_cond_timedwait(&mCaptureSyncCond, &mCaptureSyncMutex,&tv) != 0) {
           TEST_ERR("ERROR: nonZSL snapshot Timeout \n");
           res = -1;
       }
       pthread_mutex_unlock(&mCaptureSyncMutex);
       stop();
