...
int main(int argc, char *argv[])
{
    ...
    while (!stop){
        ...
        case 'P':{
            ...
            if (HAL3Test[CurCameraId]->mConfig->mTestMode == TESTMODE_SNAPSHOT) {
                ...
                testVideo->dumpPreview(num);
                testVideo->dumpVideo(num);
            } else if (HAL3Test[CurCameraId]->mConfig->mTestMode == TESTMODE_TOF) {
                QCameraHAL3TestTOF* testTof =
                    (QCameraHAL3TestTOF *)HAL3Test[CurCameraId];
        ...
        case 'D':{
            ...
            HAL3Test[CurCameraId] = NULL;
	    if(Camera[CurCameraId])
            	Camera[CurCameraId]->mDevice->closeCamera();
	    Camera[CurCameraId] = NULL;
	...
	case 'C': {
           printf("change:\n");
           TestConfig* testConf = new TestConfig();
           res = testConf->parseCommandlineChange(1, (char*)param.c_str());
           if (res != 0) {
	       printf("error command order res:%d\n", res);
	   }
           CurCameraId = testConf->mCameraId;
           printf("CurCameraId", CurCameraId);
           if (NULL == Camera[CurCameraId]) {
	       Camera[CurCameraId] = new QCameraHAL3Base(CamModule,CurCameraId);
	   }
